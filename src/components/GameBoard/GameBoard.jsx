...
const GameBoard = () => {
	...
	return (
		<DndContext
			...
			<section
				id="gameboard"
				className={`${layout} ${isLoading ? 'loading' : ''} ${hint ? 'hint' : ''}`}>
				<Pieces type="movie" />
				<PlayArea />
				<Pieces type="star" />
				<nav>
					...
					<motion.button
						...
						aria-label="View Rules or Get Hint">
						<CircleHelp />
					</motion.button>
				</nav>
	...
	function handleDragEnd(e) {
		...
		if (idxKey) {
			...
			if (
				...
				const validDrop =
					e.collisions[0]?.data?.droppableContainer?.data?.current?.accepts?.includes(dragType);
				if (validDrop) {
